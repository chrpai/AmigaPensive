...
void main()
{
	...
	if ((NoviaSysBase = OpenLibrary("noviasys.library", 0)))
	{
		...
		if (cport && cport->currentdir)
		{
			...
			ioprintf("S|ID|Title                                        Size|  Flags |         Date      \n");
	//		          *001 1234567890123456789012345678901234567890 00000000 SIZE---- 12-12-1999 01:01:01
	/*		while ((ieb = (ItemEntryBlock *)RemHead((List *)&cport->itementrylist)))
			{
				currentieb = (ItemEntryBlock *)newlist.lh_Head;
				quit = FALSE;
				while (!quit)
				{
					cmpresult = Stricmp(currentieb->Title, ieb->Title);
					if (cmpresult > 0)  // currentieb > ieb
					{
					}
					else
					{
						...
						{
							if (cmpresult == 0)	// currentieb == ieb
								Insert((List *)&newlist, (Node *)ieb, (Node *)currentieb);
						}
					}
				}
				AddTail(&newlist, (Node *)ieb);
			}*/
			while (ieb->ln_Succ)
			{
				strcpy(bits,"--------");
