 ...
 * Adapted for WWBBS by Michael Bruyere *
...
say ''spaces''pen4'$$                                      $$'def''cr
say ''spaces''pen4'$$'def' 'pen6'adapted for WWBBS by Michael Bruyere'def' 'pen4'$$'def''cr
say ''spaces''pen4'$$                                      $$'def''cr
say ''spaces''pen4'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'def''cr
say ''cr
time=time();date=date()
...
SAY 'Tap 'pen3'Return'def' to continue.'
...
PROMPT:
say ''cr
SAY ''pen2'Enter Your Stock Market Option?'def' > '
result=readstr()
choice=upper(result)
CALL checkBBS()
...
say ''cr;SAY 'Tap 'pen3'Return'def' to continue.'
...
call NEGCASH
say ''cr;SAY ''pen3'Are you sure (Y/n)?'def' > '
...
SAY 'Enter the Number of the Company you wish to check > '
result=readstr();i=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
say ''cr;SAY 'Do you wish to BUY or SELL Stocks > '


result=readstr();choice=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
SAY 'Enter Number of the Company you wish to Buy > '
result=readstr();i=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
SAY 'Enter ''A'' to buy the MAXIMUM amount > '
result=readstr();buyme=upper(result)
if result='###PANIC' then SIGNAL GETOUT
if buyme='A' then do
...
say ''cr;SAY ''pen5'Execute purchase order (Y/n)?'def' > '
...
SAY 'Enter ''A'' to sell all your Shares of 'cn$.i' > '
result=readstr();sellme=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
say ''cr;SAY ''pen3'Execute sell order (Y/n)?'def' > '
...
if exists(file8) then do
  ...
  say ''cr
  SAY'Hit 'pen3'return to continue'def''
  result=readstr()
  say ''cr
  end
...
say ''cr;SAY 'Tap 'pen3'Return'def' to continue.'cr
...
arg shfl
  ...
  say ''cr
  SAY'Hit 'pen3'return'def' to continue or 'pen6'Q'def' to quit >'
  result=readstr()
...
say ''cr;SAY 'Your choice,' name '> '


result=readstr();mci=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
SAY ''pen2'Enter the Number of the Company you wish to check'def' > '
result=readstr();i=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
say ''cr;SAY 'Enter companystat you wish to change > '


result=readstr();di=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
if di=1 then do
  ...
  SAY ' > ';result=readstr();newcn$=result
  if result='###PANIC' then SIGNAL GETOUT
  cn$.i=newcn$
  end
if di=2 then do
  ...
  SAY 'Enter new Current Price > '
  result=readstr();cv.i=result
  if result='###PANIC' then SIGNAL GETOUT
  end
if di=3 then do

  SAY 'Enter new Initial Price > '
  result=readstr();aa.i=result
  if result='###PANIC' then SIGNAL GETOUT
  end
if di=4 then do


  SAY 'Enter the new number of Shares Free > '
  result=readstr();sf.i=result
  if result='###PANIC' then SIGNAL GETOUT
  end
if di=5 then do


  SAY 'Enter the new Trend Value > '
  result=readstr();tq.i=result
  if result='###PANIC' then SIGNAL GETOUT
  end
if di=6 then do


  SAY 'Enter new Risk Value > '
  result=readstr();rv.i=result
  if result='###PANIC' then SIGNAL GETOUT
  end
signal SYSOPMENU
...
SAY 'Enter the number of the player you wish to delete > '
result=readstr();delplyr=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
say ''spaces'[X]  Exit to Sysop Menu'cr
say ''cr;SAY 'Enter the number of the file to view > '


result=readstr();vfile=upper(result)
if result='###PANIC' then SIGNAL GETOUT
...
ONELINER:
say ''cr
SAY ''pen6'Leave a one line tip for the next Investor (y/N)?'def' > '


result=readstr();ans=upper(result)
...
SAY''pen2'Enter a ONE LINE tip:'def' ';say ''cr

result=readstr(); mess=result
if result='###PANIC' then signal BYE
...
SAY ''pen4'Does this look OK?'def' > '
...
ALIAS:
say ''cr
SAY ''pen6'Do you wish to use an 'pen3'ALIAS 'pen6'(Y/n)? >'def' '
...
say ''pen5'Yes'def''cr
say ''cr
SAY''pen2'Enter your 'pen3'ALIAS 'pen5'(Max 20 chars):'def' '
result=readstr()
say ''cr
...
say ''cr
SAY ''pen2'Does this look OK (Y/n)?'def' > '
