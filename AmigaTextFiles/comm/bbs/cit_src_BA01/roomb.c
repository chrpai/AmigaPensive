...
getYesNo(char *prompt)
{
  ...
  input = ConOnly ? ( int (*)(void) )getCh : ( int (*)(void) )iChar;
...
knownHosts(char *name)
{
  ...
  if (i == ERROR)
    if ((i = ListAsShared(name)) == ERROR)
      return TRUE;
  CSetMode(netBuf.netRooms[i].mode, ShType);
  putNet(slot, &netBuf);
  return TRUE;
...
addToList(char *name)
{
  int       slot,
            i;
  if (cfg.BoolFlags.debug) splitF(NULL, " addToList(%s)\n", name);
...
ListAsShared(char *name)
{
  int       i,
            temp,
            gen;
  if (cfg.BoolFlags.debug) splitF(NULL, "ListAsShared(%s)\n", name);

  for (i = 0; i < SHARED_ROOMS; i++)
    {
      if (cfg.BoolFlags.debug)
         splitF(NULL, "netBuf.netRooms[%d].srgen:%08x\n"
              ,i,netBuf.netRooms[i].srgen);
      ...
      */
          ...
          if (cfg.BoolFlags.debug)
            {
            splitF(NULL, "netBuf.netRooms[%d].srslot:%08x  roomTab[%d].rtgen=%08x "
              ,i,netBuf.netRooms[i].srslot, temp, roomTab[temp].rtgen);
            splitF(NULL, "gen:%d shared:%s inuse:%s "
              , roomTab[temp].rtflags.SHARED == 0 ? "NO" : "YES"
              , roomTab[temp].rtflags.INUSE  == 1 ? "YES" : "NO");
            };
  ...
  CSetMode(netBuf.netRooms[i].mode, PEON);
  return i;
...
searchForRoom()
{
  ...
  if (cfg.BoolFlags.debug) splitF(NULL, "searchForRoom:%s)\n", roomBuf.rbname);
...
killFromList(char *sysName)
{
  int       i,
            slot;
  if (cfg.BoolFlags.debug) splitF(NULL, "KillFromList(%s)\n", sysName);
 ...
 */
  if (cfg.BoolFlags.debug)
     splitF(NULL, " CmnNetList(%s,%d,%s,%s);\n",name,slot,(ShouldBeThere==TRUE)?"T":"F", errstr);
