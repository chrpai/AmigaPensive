...
int main(int argc, char **argv)
  ...
  cfg.paramVers,VERSION_NAME, COPYRIGHT);
  ...
  if (!(SetVal = readSysTab(FALSE, FALSE)))
    ...
    if (onlyParams)
      {
      printf("'onlyParams' parameter ignored, unable to read CTDLTABL.SYS\n");
      onlyParams = FALSE;
  ...
  if ((fBuf = fopen("ctdlcnfg.sys", READ_TEXT)) == NULL)
    {
    /* ASCII mode   */
    printf("Error: Can't open ctdlCnfg.sys!\n");
    perror("fopen");
    poserr("fopen");
    exit(1);
...
void logInit()
  ...
  for (thisLog = 0;  thisLog < cfg.MAXLOGTAB;  thisLog++)
    ...
    if (logBuf.lbflags.L_INUSE == 1)
      ...
      **/
      Fix_User_Name(logBuf.lbname);
      }
    else
      {
      printf("%-25s \r","<not in use>");
      };
