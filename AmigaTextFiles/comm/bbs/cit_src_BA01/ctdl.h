...
struct MiscBool
  ...
  BOOLEAN_FLAG(tdebug);       /* TRUE if special debug is turned on    */
  /**
    default values for when someone is not logged in, or what a
    new user will get.  Values set by CONFG only.

    #DEFAULT-DATE-FORMAT  [STANDARD|LONG|SHORT]

    DFDATE     LGDATE     Display of 2000 jan 01
      1          0            00JAN01  -  SHORT
      1          1          2000JAN01  -  LONG
      0          0           100JAN01  -  STANDARD ( default if not specified )
      0          1           100JAN01  -  STANDARD
  **/
  BOOLEAN_FLAG(DFDATE);      /* Use default 2/3 digit date if not set */
  BOOLEAN_FLAG(LGDATE);      /* full 4 digit date if set, 2 digit if not */
  BOOLEAN_FLAG(unused23);     /* unused bit in the 32 bit word of flags */

  BOOLEAN_FLAG(unused24);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused25);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused26);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused27);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused28);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused29);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused30);     /* unused bit in the 32 bit word of flags */
  BOOLEAN_FLAG(unused31);     /* unused bit in the 32 bit word of flags */
  };
/* Let's begin by defining the configuration struct.    */
/* This is part of the contents of ctdltabl.sys         */
#define MAX_DIAL_STRINGS  (7)
typedef struct
  {
  ...
  UNS_16  DialPrefixes[MAX_DIAL_STRINGS], netSuffix;
...
struct lflags
  ...
  /**
    DF_DATE   LG_DATE     Display of 2000 jan 01
      1          0            00JAN01
      1          1          2000JAN01
      0          0           100JAN01
      0          1           100JAN01
  **/
  BOOLEAN_FLAG(DF_DATE);      /* Use default 2/3 digit date if not set */
  BOOLEAN_FLAG(LG_DATE);      /* full 4 digit date if set, 2 digit if not */
  BOOLEAN_FLAG(lflag7);
  BOOLEAN_FLAG(lflag8);
  BOOLEAN_FLAG(lflag9);
...
#define CGetMode(x)  ((x) & 7)
#define CSetMode(x, y) x = (x & (~7)) + y;
#define GetFA(x)  ((x) & 8)
#define SetFA(x)  x |= 8;
...
/*#define NO_PWD    3    */
/*#define FOUND   4      */
typedef enum { NO_ROOM=0, NOT_SHARING=1, NOT_SHARED=2, NO_PWD=3, FOUND=4, RS_INIT} roomSearchType;
...
typedef struct
  ...
  int   room;
  int   index;
  roomSearchType  reason; /* what this call did */
  }
RoomSearch;
...
#define initLogBuf(x)   (x)->lbgen = (AN_UNSIGNED *) GetDynamic(GEN_BULK),\
                        (x)->lbMail = (theMessages *)GetDynamic(MAIL_BULK)
#define killLogBuf(x)   free((x)->lbgen), free((x)->lbMail)

#define initRoomBuf(x)  (x)->msg = (theMessages *)GetDynamic(max(MAIL_BULK, MSG_BULK))

#define killRoomBuf(x)  free((x)->msg)
#define initNetBuf(x)   (x)->netRooms = (SharedRoom *)GetDynamic(sizeof (*(x)->netRooms) * SHARED_ROOMS)
