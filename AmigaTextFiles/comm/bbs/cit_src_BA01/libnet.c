...
void getNet(int n, NetBuffer *buf)
  ...
  if (SR_BULK != 0)
    {
    if (fread(buf->netRooms, SR_BULK, 1, netfl) != 1)
      {
      sPrintf(error, "?getNet-read fail(2) for slot %d.", n);
      crashout(error);
...
void putNet(int n, NetBuffer *buf)
  ...
  memcpy(netTab[n].netTRooms, buf->netRooms, (long)SR_BULK);
  ...
  if (SR_BULK != 0)
  if (fwrite(buf->netRooms, SR_BULK, 1, netfl) != 1)
    {
    sPrintf(error, "?putNet-write fail(2), slot %d!!", n);
    crashout(error);
...
* normId()
*
* This normalizes a node id, i.e., kills all the punctuation and uppercases
* the country designation.
*/
char normId(label source, label dest)
  {
  int digitcount = 0;
  while ( !isalpha( *source ) && *source )  source++;
