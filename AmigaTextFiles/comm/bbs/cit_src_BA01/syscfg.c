...
int sysSpecs(char *line, int offset, char *status, FILE *fd)
  ...
  *status = TRUE;
  if (sscanf(line, "%s %s ", var, string))
    {
    arg = ( string[0] == '\"' ) ?   0  : atoi(string);
    if (strCmpU(var, "#HELPAREA"  )    == SAMESTRING)
      {
      doAreaCommon(var, line, &cfg.homeArea, HELP);
      }
    else if (strCmpU(var, "#DEFAULT-DATE-FORMAT" ) == SAMESTRING )
      {
      char tempbuf[64];
      readString(line, &tempbuf[0], FALSE);
      if( strCmpU(tempbuf, "STANDARD") == SAMESTRING )
        {
        cfg.BoolFlags.DFDATE = 0;
        cfg.BoolFlags.LGDATE = 0;
        }
      else if( strCmpU(tempbuf, "SHORT") == SAMESTRING )
        {
        cfg.BoolFlags.DFDATE = 1;
        cfg.BoolFlags.LGDATE = 0;
        }
      else if( strCmpU(tempbuf, "LONG") == SAMESTRING )
        {
        cfg.BoolFlags.DFDATE = 1;
        cfg.BoolFlags.LGDATE = 1;
        }
      else
        illegal("The #DEFAULT-DATE-FORMAT parameter must be STANDARD, SHORT, or LONG");
      }
    ...
    else if (strCmpU(var, "#AREXXPORT") == SAMESTRING)
      {
      readString(line, cfg.DepData.ARexxPort, FALSE);
...
void initSysSpec()
  ...
  strcpy(cfg.DepData.ARexxPort, DEFAULT_AREXX_PORT );
  }
