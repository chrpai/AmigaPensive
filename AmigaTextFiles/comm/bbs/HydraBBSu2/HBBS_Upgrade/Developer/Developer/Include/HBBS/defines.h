...
#define GL_NOINACTIVITY 4096 // Disables Inactivity checking (primarily for use with awaitconnect door)
#define GL_COUNTDOWN    8192 // Displays a countdown if used with a timeout value!
...
#define IN_DIALOUT 10  // if sysop presses F4 at await connect prompt you'll get this..

#define IN_PAGEANSWERED 11 // only returned if GL_NOTIFYPAGE was specified and the sysop
                           // entered chat mode.
...
#define DCP_NONE      0  // accept internal defaults (subject to change at my descression)
#define DEFAULT_NONE  0  // as above
#define DEFAULT_YES   1  // set the default to YES
#define DEFAULT_NO    2  // set the default to NO
#define DCP_ADDYN     4  // add's [Y/n] or [y/N] accordingly.
#define DCP_COUNTDOWN 8  // adds a countdown timer
#define DCP_CLEAR     16 // removes the prompt line after continuing.
#define DCP_NODISPLAY 32 // does not display Y or N when the user presses a key.
#define DCP_COLOURIZE 64 // passes the prompt string through MenuPrompt before displaying it
...
#define LAST_ACTION              16    // must be last ACTN_blah + 1
