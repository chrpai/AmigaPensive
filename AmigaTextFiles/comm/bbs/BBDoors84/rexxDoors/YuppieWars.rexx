/* YUPPIE WARS!  V2.6-bb5 22-Nov-90/21-Feb-91  -  Dave Cole */
...
/* Updated for BBBBS or CLI by Richard Stockton 13-Aug-94 (bb5) */
...
IF handle='' THEN
  DO
    ...
    OPTIONS PROMPT 'Do You Want Color Graphics? (nY) > '
    PULL gfx 
...
IF x~=0 THEN
  DO
    ...
    bbspath=WORD(line,1)
  END
CALL PRAGMA('W','W')  /* disk requesters ON */
IF bbspath~='' THEN
  DO
...
if lastpl ~= date then do
   ...
   deadf = 0
   call delay(100)
   OPTIONS PROMPT RESET"Press "RED"RETURN"RESET" when finished: "RESET
   PULL junk
...
CALL MESSAGE CYA center("Version 2.6-bb5",78)
CALL MESSAGE " "
...
call delay(100)
OPTIONS PROMPT RED center("--= Press Return =-- ",78)||RESET
PULL junk
if checkBBS() then signal BYE
...
CALL MESSAGE " "
CALL MESSAGE " "
OPTIONS PROMPT MAG"Need Instructions ("GRE"y"MAG"/"RED"N"MAG")? "RESET
PULL info
if checkBBS() then signal BYE
 if info = "Y" then do
  do i = 1 to 5
     ...
     call close(instr)
     OPTIONS PROMPT " Press "RED"RETURN"WHI" when finished: "RESET
     PULL junk
     if checkBBS() then signal BYE
     CALL MESSAGE " "
    end
...
CALL MESSAGE " "
OPTIONS PROMPT MAG"Read The News ("GRE"y"MAG"/"RED"N"MAG")? "RESET
PULL newz
if checkBBS() then signal BYE
if newz = "Y" then do
  q = 0
  ...
  IF x~=0 THEN do until EOF(newz)
     ...
     if q // (LINES - 3) = 0 then do
        OPTIONS PROMPT RESET"Hit "RED"RETURN"RESET" to continue: "RESET
        PULL ans
        if checkBBS() then signal BYE
        if ans = "Q" then do
           call close(newz)
...
CALL MESSAGE " "
OPTIONS PROMPT BLU"["YEL||turnsleft||BLU"]"CYA" Your Choice?"BLU"> "RESET
PULL Choice 
if checkBBS() then signal BYE
...
if turnsleft <= 0 then do
   ...
   CALL MESSAGE " "
   OPTIONS PROMPT RESET"Press "RED"RETURN"WHI" when finished: "RESET
   PULL junk
   signal OUTTAHERE
  end
CALL MESSAGE CLS
...
CALL MESSAGE " "
CALL MESSAGE " "RESET
OPTIONS PROMPT BLU"["YEL"D"BLU"]"CYA" Deposit, "BLU"["YEL"W"BLU"]"CYA" Withdraw, "BLU"["YEL"L"BLU"]"CYA" Leave Bank"BLU"> "RESET
PULL ans 
if checkBBS() then signal BYE
...
WITH:
CALL MESSAGE " "
OPTIONS PROMPT GRE"Withdraw How Much Money?"BLU"> "RESET
PULL wit 
if checkBBS() then signal BYE
...
DEPO:
CALL MESSAGE " "
OPTIONS PROMPT GRE"Deposit How Much Money?"BLU"> "RESET
PULL dep 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Enter the arena? ("GRE"Y"CYA"/"RED"n"CYA")"BLU"> "RESET
PULL answ 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Fight which one? "BLU"["YEL"Q"BLU"]"CYA" Quits"BLU"> "RESET
PULL answ 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Fight Commands: "BLU"["YEL"A"BLU"] "WHI"Attack, "BLU"["YEL"R"BLU"] "WHI"Retreat, "BLU"["YEL"H"BLU"]"WHI" Hit his vehicle"BLU"> "RESET
PULL opt 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Do you wish to fight another player ("GRE"Y"CYA"/"RED"n"CYA")? "RESET
PULL ans 
if checkBBS() then signal BYE
if ans = "Q" then signal MENU
...
CALL MESSAGE " "
CALL MESSAGE " "
OPTIONS PROMPT GRE"Enter your opponents name: "RESET
PULL who 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Oh great one, what is your command?"BLU"> "RESET
PULL ans 
if checkBBS() then signal BYE
...
if exists(datapath||ss) then do
   CALL MESSAGE " "
   OPTIONS PROMPT CYA"Delete "WHI||ss||CYA" ("GRE"y"CYA"/"RED"N"CYA")? "RESET
   PULL st 
   if checkBBS() then signal BYE
   if st = "Y" then do
...
CALL MESSAGE " "RESET
OPTIONS PROMPT CYA"Where do you want to travel?  "BLU"["YEL"Q"BLU"]"CYA" Quits:"BLU"> "RESET
PULL trav 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"Wanna go? ("GRE"Y"CYA"/"RED"n"CYA"):"BLU"> "RESET
PULL ans 
if checkBBS() then signal BYE
...
if ransak = 13 | ransak = 21 | ransak = 192 | ransak = 112 | ransak = 80 then do
   ...
   CALL MESSAGE " "
   OPTIONS PROMPT CYA"Sue the pilot (Y/n)? "
   PULL ans 
   if checkBBS() then signal BYE
   if ans = "N" then do
...
if money = payme then do
   ...
   CALL MESSAGE " "
   OPTIONS PROMPT CYA"Stay here anyway ("GRE"Y"CYA"/"RED"n"CYA")? "RESET
   PULL stayany 
   if checkBBS() then signal BYE
...
CALL MESSAGE " "RESET
CALL MESSAGE " "
OPTIONS PROMPT " Hit "RED"RETURN"WHI" when finished: "RESET
PULL junk
if checkBBS() then signal BYE
signal MENU
...
CALL MESSAGE " "RESET
OPTIONS PROMPT CYA"What game do you want to play?"BLU"> "RESET
PULL Choice 
if checkBBS() then signal BYE
if Choice = "Q" then do
turnsleft = turnsleft + 1
...
CALL MESSAGE " "
OPTIONS PROMPT BLU" You have $"CYA||money||BLU", your bet (Max $"CYA"5000"BLU")?"RESET
PULL Betty 
if checkBBS() then signal BYE
...
OPTIONS PROMPT CYA" How much money do you want to bet?: "RESET
PULL Wager 
if checkBBS() then signal BYE
...
WEZ:
CALL MESSAGE " "
OPTIONS PROMPT CYA" Bet "YEL||Wager||CYA" ("GRE"Y"CYA"/"RED"n"CYA")?: "
PULL okeydokey 
if checkBBS() then signal BYE
...
OPTIONS PROMPT MAG"   Enter a number between 1 & 50: "
PULL numb 
if checkBBS() then signal BYE
...
CALL MESSAGE CLS
CALL MESSAGE " "
OPTIONS PROMPT CYA" Would you like to play again ("GRE"y"CYA"/"RED"N"CYA")? "
PULL comegen
if checkBBS() then signal BYE
...
*/
CALL MESSAGE " "
OPTIONS PROMPT GRE" Buy or Sell your weapon (B/S)? "RESET
PULL Choice 
if checkBBS() then signal BYE
...
*/
CALL MESSAGE " "
OPTIONS PROMPT GRE" Buy or Sell your vehicle (B/S)? "RESET
PULL Choice 
if checkBBS() then signal BYE
...
CALL MESSAGE " "
OPTIONS PROMPT CYA"You have $"WHI||money||CYA".  What weapon do you want?"BLU"> "RESET
PULL Choice 
if checkBBS() then signal BYE
if Choice = "Q" then signal MENU
if Choice = "A" then do
...
CALL MESSAGE WHI"                               Press Q To Quit      "
CALL MESSAGE " "
OPTIONS PROMPT CYA"You have $"WHI||money||CYA".  What vehicle do you want?"BLU"> "RESET
PULL Choice 
if checkBBS() then signal BYE
if Choice = "Q" then signal MENU
if Choice = "A" then do
...
CALL MESSAGE " "
OPTIONS PROMPT YEL"Are You Sure (y/N)? "RESET
PULL Choice 
if checkBBS() then signal BYE
CALL MESSAGE " "
if Choice = "Y" then do
...
P2:
CALL MESSAGE " "
OPTIONS PROMPT WHI"O"RED"k"GRE" D"YEL"a"BLU"v"MAG"i"CYA"d"WHI","RED" e"GRE"n"YEL"t"BLU"e"MAG"r"CYA" p"WHI"a"RED"s"YEL"s"BLU"w"MAG"o"CYA"r"WHI"d "RED"#"GRE"2"YEL"!"BLU": "
PULL pass 
if checkBBS() then signal BYE
...
QUIT:
CALL MESSAGE " "
OPTIONS PROMPT CYA"Are you sure ("GRE"n"CYA"/"RED"Y"CYA")? "RESET
PULL Choice 
if checkBBS() then signal BYE
if Choice~='N' then do
...
CALL MESSAGE " "
OPTIONS PROMPT MAG"You have $"money" "BLU"["YEL"Q"BLU"]"MAG" Quits -- Your bet (Max 5000)?"BLU"> "RESET
if checkBBS() then signal BYE
PULL Bet 
...
BEANS:
OPTIONS PROMPT POS4 CLL POS4 CYA"Press 'P' to pull for your chance at big bucks! > "
PULL chr 
if checkBBS() then signal BYE
if chr = 'P' then do
...
WHATTHEHELL:
CALL MESSAGE " "
OPTIONS PROMPT POS17 CLL POS17 "Do you want to continue playing (Y/N)? "
PULL ans 
if checkBBS() then signal BYE
...
Bet:
        CALL MESSAGE " "
                ...
                OPTIONS PROMPT CYA" How much do you wish to bet "BLU"["YEL"Q"BLU"]"CYA" To Quit?: "RESET
                PULL TimeBet 
                if checkBBS() then signal BYE
...
RollIt:
   do forever
        CALL MESSAGE " "
        OPTIONS PROMPT "Press ["RED"RETURN"RESET"] To Roll... "
        PULL YesPlay 
        if checkBBS() then signal BYE
        call Roller
    end
...
Again:
        CALL MESSAGE " "
        OPTIONS PROMPT GRE"Do You Want To Play Again (Y/N)? "RESET
        PULL PlayAgain
        if checkBBS() then signal BYE
