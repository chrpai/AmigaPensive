...
struct ScreenInfoData
{
    ...
    V_BOOL UsePubScreen;
    V_BOOL UseBackfill; // the user sets this ...
    V_BOOL UsingBackfill; //and control will set this if the backfill is required
    UWORD *ScrPens;
};
...
// where the B means that the default (and therefore highlighted) option
// will be "Bananas"
//
// then using the data in BBSCols we can change the output of the prompt
// passed to MenuPrompt() before we display it

struct BBSColsData
{
  V_STRING MenuTextANSI;            // default: \033[0;37m    "white"
  V_STRING MenuOpenBracket;         // default: \033[0;34m[   "blue + ["
  V_STRING MenuCloseBracket;        // default: \033[0;34m]   "blue + ]"
  V_STRING MenuHighlightANSI;       // default: \033[0;33m    "yellow"
  V_STRING MenuDefaultOptANSI;      // default: \033[1;37m    "bold + white"
  V_STRING MenuPromptANSI;          // default: " \033[0;34m: \033[0;37m"  "space + blue + : + space + white" Note: the white makes the default text colour of calls to GetLine() white, after a call to MenuPrompt()
  V_STRING MenuDefaultOptTextANSI;  // default: \033[0;33m    "yellow"
};

// using the defaults above with the example above, the string that gets
// printed looks like this:
//
// "\033[0;37mFruit \033[0;34m[\033[0;33mO\033[0;34m]\033[0;37mranges, \033[0;34m[\033[1;37mB\033[0;34m]\033[0;37mananas, \033[0;34m[\033[0;33mA\033[0;34m]\033[0;37mpples \033[0;34m: \033[0;37m"
//
...
// door coders can also use the variables above in their own prompts, like this:
// sprintf(outstr,"My prompt",N_ND->BBSCols->MenuPromptANSI);
...
struct ConfData
  {
    ...
    V_STRINGLIST  UserNotAllowed; // list of users who are NOT allowed, or NULL
                               // if you want to allow every user access to the conf
                               // takes precedence over UserAllowed option
...
struct UserData
  {
    ...
    V_CHAR     ComputerType[LEN_COMPUTERTYPE+1];  // note: in HBBS this is a string, not a number!

    V_CHAR     SentBy[LEN_SENTBY+1]; // sentby string!
...
struct NodeData
  {
    ...
    ULONG SecInfo; // private!

    V_BOOL KeyOK; // boolean, if hydrabbs is registered this will be set to true
...
//    V_BIGNUM NewUserConf;
    ...
    V_BIGNUM CallsThisWeek; // total amount of calls bbs has received this week!
    UBYTE LastWeekStartDate[LEN_DATESTR+1];
};
