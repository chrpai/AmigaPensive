...
do forever
	SetStatus "QWK Settings:"
	if GetUserMisc(ln, "QWKASYNC") = 1 then des = "ON"
	else des = "OFF"
	SendModem CRLF||"(1)  "||CYAN||"Asynchronous operation: "||WHITE||des||CRLF
	des = GetUserMisc(ln, "QWKUNARC")
	select
	...
	cmdstr = AskInput(ln, "("||GetTimeLeft(ln)||") "||CYAN||"QWK Settings: "||WHITE, "", 255)
	sendmodem CRLF
	...
	select
		when cmdid = "1" then do
			if GetUserMisc(ln, "QWKASYNC") = 1 then do
		...
		when cmdid = "2" then do
			...
			call SetUserMisc ln, "QWKUNARC", des
			end
		when cmdid = "3" then do
			...
			call SetUserMisc ln, "QWKARC", des
			end
