...
void DoorMain( char *DosArgs )
{
   ...
   if( !(CNet4Base = OpenLibrary("cnet4.library", 4)) )
	...
	while(CNC_EnterLine(40, ELINE_INPUTBOX, "Enter a CNet range: "))
		{
		if(CNetFindRange(z->InBuffer, 1, 100, &rc))
			...
			for(;;)
				{
				x=CNetNextRange(&rc);
				sprintf(z->ABuffer, "value: %4.4ld    nparts: %2ld    set: %2ld    element: %4ld\n", x, rc.nparts, rc.set, rc.element);
				CNC_PutA();
				if( (rc.set == (rc.nparts-1)) && (rc.element == (rc.parts[rc.set][1])) )
					{
					CNC_PutText("END OF RANGE!\n");
					exit(0);
					}
				}
			}
