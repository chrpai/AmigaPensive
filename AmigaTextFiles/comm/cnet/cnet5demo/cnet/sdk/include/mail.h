...
struct MailHeader4 {				// NEW 2.0 for mail, etc
	...
	ULONG IDate;					// Date inported! Used for sorting!
	
	ULONG RFCHead;					// number which is to be ASCII coded
										// and used as the filename for the RFC822
										// header for this email..  eg. "mail:users/<mail_id>/HEADER/<822Number>"
										// (only valid if non-NULL)
	
	char	Expansion[118];		// future expansion
	short	Recipients;				// number of recipients ...
...
struct MailAlias4 {
	...
	char Address[80];					// 48 - destination address including "@" - blank if destination is local system
...
struct MailToList
{
	...
	UBYTE MailType;   // type of mail object - mailtask uses this to decide what
	...
	char fileattach[128]; // path to file attached to this mail.
	struct MailToList *next;
};

#define MAILTO_REALNAME		0 // local REALNAME
#define MAILTO_HANDLE		1 // local HANDLE
#define MAILTO_UUCP			2 // UUCP/Internet (use mail ID)
#define MAILTO_FIDO			3 // FIDO (use nick or realname)
#define MAILTO_ALIAS			4 // local ALIAS
#define MAILTO_FAHANDLE		5 // Handle from FindAccount
#define MAILTO_FBACK			6 // Feedback
#define MAILTO_NUSER			7 // Newuser
#define MAILTO_SYSOP			8 // "SYSOP"
...
struct MIMEContentType
	...
	UBYTE method;		 // 0-255; how to handle attachment
							 // current methods:   0=leave as email body text
	...
	UBYTE newsmethod;	 // 0-255; how to handle attachment
							 // current methods:   0=leave as body text
							 //                    1=undefined
							 //                    2=Save as VALIDATED file in base with GO KEY specified in ProcessArg1
							 //                    3=Save as UNVALIDATED file in base with GO KEY specified in MethodArg1
							 //                    4=save to >NIL: (eg. delete/lose it from the system)
	...
	char fileext[28];	// file extension to associate with this mime type
