/* FIDO-Net DoTick, v2.0  by Dotoran (1-Jul-94) */
options results

dfile='sysdata:log/poll' ; time='[0m[[33m'date()' 'time()'[0m> '
...
do i=1 to files ; a=upper(word(f,i))
    if right(a,4)='.TIC' then do ; call open(f2,'inbound:'||a,'r')
        ...
        if b=1 then do ; say "Processing TIC:"a ; call LOG("Processing TIC:"||a)
            ...
            call LOG("Cleaning up INBOUND: directory...") ; say "Cleaning up INBOUND: directory..."
            address command "delete inbound:"||a ; address command "delete inbound:"||file
        end
        if b=0 then do ; say
...
/* <EOF> DoTick, v2.0 by Dotoran of Frontiers BBS! (716)/823-9892         */
