...
void main( int argc, char **argv )
{
	...
	onbreak(intGetOut); // this break trap causes the program to exit when
                  // the program receives a break (CTRL-C) signal
 ...
 ************************************************************************/
	 ...
	 *                         exit back to CNet!
	 ************************************************************************/
	// Be sure to insert (or call) your cleanup code in the GetOut() function
   // below
...
/**************************************************************************
 *       An added function to call GetOut() for onbreak() support         *
 **************************************************************************/
int intGetOut( void )
{
	exit(0);
...
void GetOut( void )
{
	...
	// Anything that needs to be cleaned up when CTRL-C is sent to your
	// program, if "BREAK <tasknum>" is used in a shell to break your program
	// or cleanup required when exit() is called needs to be done here!
...
{
	/* a     = handle, name, account number to find
    * b     = pointer to instance of struct UserData, where loaded account will
    *         be placed.
    * quiet = if account not found, do not print "could not find 'a'".
    *
    * also, you can set z.DoQuick=1 BEFORE calling FindAccount() if you do
    * NOT want CNet to pop up the mini-handle/userlist.
    */
